...
int fill_request(struct kvec *vec, const char *method,
...
int64_t parse_http_response(char *raw_response, size_t raw_response_size,
  ...
  // Read Response Line
  {
    char *status_line = strsep(&buffer, "\n");
    strsep(&status_line, " ");
    if (status_line == 0) {
...
int64_t networkfs_http_call(const char *method,
  ...
  error = fill_request(&kvec, method, arg_size, args);
  va_end(args);
